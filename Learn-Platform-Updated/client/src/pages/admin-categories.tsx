...
export default function AdminCategories() {
    ...
    const saveMutation = useMutation({
        mutationFn: async (formData: any) => {
            ...
            if (!res.ok) throw new Error("فشل حفظ التصنيف");
            return res.json();
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["admin-categories"] });
            toast({
                ...
                description: selectedCategory ? "تم تحديث بيانات التصنيف" : "تم إضافة تصنيف جديد بنجاح"
            });
    ...
    const seedSpecializationsMutation = useMutation({
        ...
        onSuccess: (data: any) => {
            ...
            toast({
                title: "تمت الإضافة",
                description: `تمت إضافة ${data?.createdCount ?? 0} تصنيفات من التخصصات`,
            });
        },
        onError: (error: any) => {
            toast({ title: "خطأ", description: error.message, variant: "destructive" });
    ...
    const handleDelete = (id: string) => {
        if (confirm("هل أنت متأكد من حذف هذا التصنيف؟ الكورسات المرتبطة به ستصبح بدون تصنيف.")) {
            ...
            <div className="space-y-6" dir="rtl">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground">التصنيفات تساعدك على تنظيم الكورسات.</p>
                    </div>
                    ...
                    <form onSubmit={handleFormSubmit} className="space-y-4 py-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Textarea
                                ...
                                placeholder="اشرح ما يتضمنه هذا التصنيف..."
                            />
                        </div>
