...
const PRICE_FULL = 50;
const DISCOUNT_MAP: Record<string, { price: number, link: string }> = {
  "TAALLUM99": { price: 0.5, link: "" }, // Waiting for link
  "TAALLUM90": { price: 5, link: "" },   // Waiting for link
  "TAALLUM50": { price: 25, link: "" },  // Waiting for link
  "TAALLUM35": { price: 32.5, link: "" }, // Waiting for link
  "TAALLUM10": { price: 45, link: "" },  // Waiting for link
};
const DEFAULT_LINK = "https://www.paypal.com/ncp/payment/CW7JB9W9SH9NE";
...
export default function AIPricing() {
  ...
  const [activeDiscount, setActiveDiscount] = useState<{ price: number, link: string } | null>(null);
  const [promoError, setPromoError] = useState("");
  const [, setLocation] = useLocation();
  const queryClient = useQueryClient();
  const { toast } = useToast();

  const currentPrice = activeDiscount ? activeDiscount.price : PRICE_FULL;
  const currentLink = (activeDiscount && activeDiscount.link) ? activeDiscount.link : DEFAULT_LINK;

  const handleApplyPromo = () => {
    const code = promoInput.trim().toUpperCase();
    if (DISCOUNT_MAP[code]) {
      setActiveDiscount(DISCOUNT_MAP[code]);
      setPromoError("");
      toast({ title: "تم تطبيق الخصم!", description: `السعر الجديد هو $${DISCOUNT_MAP[code].price}` });
    } else {
                  ...
                  {!activeDiscount ? (
                    ...
                    <div className="w-full mb-4">
                      <div className="flex items-center justify-center gap-2 bg-green-500/10 border border-green-500/20 rounded-lg p-2">
                        <span className="text-green-600 text-sm font-black">{promoInput}</span>
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        if (currentLink) {
                          ...
                          toast({ title: "عذراً", description: "رابط الدفع لهذا الخصم جاري تجهيزه.", variant: "destructive" });
                        }
                      }}
                    >
